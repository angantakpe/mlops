...
- Lessons: https://codehornets.com/
- Code: [codehornets/mlops](https://github.com/angantakpe/mlops)

<a href="https://codehornets.com/#course">
...
Be sure to go through the [course](https://codehornets/#course) for a much more detailed walkthrough of the content on this repository. We will have instructions for both local laptop and Anyscale clusters for the sections below, so be sure to toggle the ► dropdown based on what you're using (Anyscale instructions will be toggled on by default). If you do want to run this course with Anyscale, where we'll provide the **structure**, **compute (GPUs)** and **community** to learn everything in one day, join our next upcoming live cohort → [sign up here](https://4190urw86oh.typeform.com/codehornets)!
...
<details open>
  ...
  We can create an [Anyscale Workspace](https://docs.anyscale.com/develop/workspaces/get-started) using the [webpage UI](https://console.anyscale.com/o/codehornets/workspaces/add/blank).
...
Start by exploring the [jupyter notebook](notebooks/codehornets.ipynb) to interactively walkthrough the core machine learning workloads.
...
<details>
  ...
  ```bash
  # Start notebook
  jupyter lab notebooks/core.ipynb
```
...
```bash
export MODEL_REGISTRY=$(python -c "from core import config; print(config.MODEL_REGISTRY)")
...
```bash
export EXPERIMENT_NAME="llm"
export RUN_ID=$(python core/predict.py get-best-run-id --experiment-name $EXPERIMENT_NAME --metric val_loss --mode ASC)
...
export EXPERIMENT_NAME="llm"
export RUN_ID=$(python core/predict.py get-best-run-id --experiment-name $EXPERIMENT_NAME --metric val_loss --mode ASC)
python core/predict.py predict \
    --run-id $RUN_ID \
...
<details>
  ...
  export EXPERIMENT_NAME="llm"
  export RUN_ID=$(python core/predict.py get-best-run-id --experiment-name $EXPERIMENT_NAME --metric val_loss --mode ASC)
  python core/serve.py --run_id $RUN_ID
  ```
...
# Coverage
python3 -m pytest tests/code --cov core --cov-report html --disable-warnings  # html report
python3 -m pytest tests/code --cov core --cov-report term --disable-warnings  # terminal report
```
...
> If you don't want to set up all of this yourself, we highly recommend joining our [upcoming live cohort](https://4190urw86oh.typeform.com/core){:target="_blank"} where we'll provide an environment with all of this infrastructure already set up for you so that you just focused on the machine learning.
...
```bash
export CLUSTER_ENV_NAME="codehornets-cluster-env"
...
```bash
export CLUSTER_COMPUTE_NAME="codehornets-cluster-compute"
...
runtime_env:
  working_dir: .
  upload_path: s3://codehornets/$GITHUB_USERNAME/jobs  # <--- CHANGE USERNAME (case-sensitive)
  env_vars:
...
ray_serve_config:
  ...
  runtime_env:
    working_dir: .
    upload_path: s3://codehornets/$GITHUB_USERNAME/services  # <--- CHANGE USERNAME (case-sensitive)
    env_vars:
      GITHUB_USERNAME: $GITHUB_USERNAME  # <--- CHANGE USERNAME (case-sensitive)
```
...
``` bash
export ANYSCALE_HOST=https://console.anyscale.com
export ANYSCALE_CLI_TOKEN=$YOUR_CLI_TOKEN  # retrieved from https://console.anyscale.com/o/codehornets/credentials
```
...
With our CI/CD workflow in place to deploy our application, we can now focus on continually improving our model. It becomes really easy to extend on this foundation to connect to scheduled runs (cron), [data pipelines](https://codehornets.com/courses/mlops/data-engineering/), drift detected through [monitoring](https://codehornets.com/courses/mlops/monitoring/), [online evaluation](https://codehornets.com/courses/mlops/evaluation/#online-evaluation), etc. And we can easily add additional context such as comparing any experiment with what's currently in production (directly in the PR even), etc.
