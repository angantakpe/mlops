...
# Get and save run ID
export RUN_ID=$(python -c "import os; from core import utils; d = utils.load_dict(os.getenv('RESULTS_FILE')); print(d['run_id'])")
...
aws s3 cp $MODEL_REGISTRY s3://codehornets/$GITHUB_USERNAME/mlflow/ --recursive
aws s3 cp results/ s3://codehornets/$GITHUB_USERNAME/results/ --recursive
